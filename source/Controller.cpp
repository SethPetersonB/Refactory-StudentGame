...
namespace Engine
{
  ...
  */
  ...
  void Controller::UpdateInput()
  {
    ...
    {
      GSM::get().getMessenger().Post("QUIT_CONFIRM", Message<std::string>("F4 pressed "));

    }
    if ((InputSystem::KeyDown(SDL_SCANCODE_LALT) || InputSystem::KeyDown(SDL_SCANCODE_RALT)) && InputSystem::KeyPressed(SDL_SCANCODE_RETURN))
    {
      Display & disp = GSM::get().getDisplay();
    ...
    if (InputSystem::KeyPressed(SDL_SCANCODE_ESCAPE)) // pause
    {
      ...
      Stage& reset = Stage::GetStage("ResetConfirmation");
      ...
      else if (quitStage.isStageRunning())
      {
        ...
        Stage::lastRunning_->setStageRunning(true);
      }
      else if (confirm.isStageRunning())
      {
        confirm.setStageRunning(false);
        pauseMenu.setStageRunning(true);
      }
      else if (options.isStageRunning())
      {
        options.setStageRunning(false);
        pauseMenu.setStageRunning(true);
      }
      else if (credits.isStageRunning())
      {
        credits.setStageRunning(false);
        pauseMenu.setStageRunning(true);
      }
      else if (reset.isStageRunning())
      {
        reset.setStageRunning(false);
        pauseMenu.setStageRunning(true);
      }
      else if (gameStage.isStageRunning() || pauseMenu.isStageRunning())
      {
    ...
    if (getParent().getStage()->isStageRunning()) // if game is not paused
    {
      ...
      if (InputSystem::KeyDown(SDL_SCANCODE_LSHIFT) && InputSystem::KeyPressed(SDL_SCANCODE_N))
      {
  ...
  */
  ...
  Controller::Controller(GameInstance* owner, const ParsedObject & obj) : Component(owner, "Controller"),
    ...
    healthId_(0), ammoId_(0), progressId_(0), waveId_(0), waveTextId_(0), waveNum_(0), waveCount_(10),
    wavesRunning_(false)
  {
    UpdateCount();
    regen_timer_.Reset();
  ...
  */
  ...
  void Controller::setWaveCount(size_t num)
  {
    waveCount_ = num;
  }
  ...
  */
  ...
  void ControllerHandler::ConnectEvents(Component * base_sub)
  {
    ...
    SUBSCRIBER_ACTION onWaveCountSet = [sub](const Packet& data) {sub->setWaveCount(data.getData<size_t>()); };
    SUBSCRIBER_ACTION wavesRunning = [sub](const Packet& data) {sub->SetWavesRunning(data.getData<bool>());};
    SUBSCRIBER_ACTION spaceVisible = [sub](const Packet& data)
    {
      GameInstance & obj = sub->getParent().getStage()->getInstanceFromID(sub->GetBeginID());

      DrawToken item = obj.RequestData<DrawToken>("Graphic");

      item.setVisible(data.getData<bool>());
    };
    ...
    objMessenger.Subscribe(objMessenger, "SetNumWaves", onWaveCountSet);
    ...
    objMessenger.Subscribe(getStage()->getMessenger(), "SetWavesRunning", wavesRunning);
    objMessenger.Subscribe(getStage()->getMessenger(), "SetSpaceVisible", spaceVisible);
    ...
    for (auto component : componentList_)
    {
      ...
      if (controller->IsWavesRunning() && regen_timer.ElapsedTime() > REGEN_RATE)
      {
        ...
        if (walls < 16)
        {
          ...
          int i = Utils::random_uniform(0,2); // needs randomness element to it...
          //The Won and Lost conditions prevent it from playing after the level is over.
          if ( i == 0 && !dynamic_cast<Controller*>(component)->isLost() && !dynamic_cast<Controller*>(component)->isWon())
          {
  ...
  */
  ...
  void Controller::autoPlay()
  {
    ...
    // get random tile
    int x = Utils::random_uniform(1, width - 1);
    int y = Utils::random_uniform(0, height - 1);

    // place random tile
    try
    {
      GameInstance& tile = stage->getInstanceFromID(grid.at(y).at(x));
